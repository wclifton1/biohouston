...
<tr>
	...
	<td>
		<?php if ( $event['event']['privacy'] == 'Private' ) : ?>
		<?php _e('Event is not listed publicly.', 'tribe-eventbrite') ?><br>
		<?php elseif ( $event['event']['privacy'] == 'Public' ): ?>
		<?php _e('Event is listed publicly on Eventbrite and search engines.', 'tribe-eventbrite') ?><br>
		<?php endif; ?>
		<small><a href="<?php echo esc_url('http://www.eventbrite.com/edit?eid='.$event['event']['id']) ?>#privacy_and_promotion_wrapper" target="_blank"><?php _e('Edit event privacy settings', 'tribe-eventbrite') ?></a></small>
	</td>
</tr>
<?php if ( isset($event['event']['tickets']) && count( $event['event']['tickets'] )) : ?>
	...
	<tr>
		<td colspan="2" class="snp_sectionheader">
			<table class="EB-table" border=0><tr><td width="120px"><?php _e('Ticket', 'tribe-eventbrite'); ?></td><td width="85px"><?php _e('Cost', 'tribe-eventbrite') ?></td><td width="40px"><?php _e('Sold', 'tribe-eventbrite'); ?></td><td width="40px"><?php _e('Available', 'tribe-eventbrite'); ?></td><td width="100px"><?php _e('End Sales', 'tribe-eventbrite'); ?></td></tr>
			<?php foreach( (array)$event['event']['tickets'] as $ticket ) : ?>
				...
				<tr>
					...
					<td><?php echo $ticket['type'] == 0 ? $ticket['currency'] . ' ' . $ticket['price'] : __( 'Donation' ); ?></td>
					<td><?php echo isset( $ticket['quantity_sold'] ) ? $ticket['quantity_sold'] : '&nbsp;'; ?></td>
					<td><?php echo isset( $ticket['quantity_available'] ) ? $ticket['quantity_available'] : '&nbsp;'; ?></td>
