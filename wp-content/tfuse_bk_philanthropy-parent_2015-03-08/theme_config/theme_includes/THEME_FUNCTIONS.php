...
if (!function_exists('tfuse_filter_get_avatar')){
    ...
    function tfuse_filter_get_avatar($avatar, $id_or_email, $size, $default, $alt){
        ...
        $avatar_src = 'http://themefuse.com/wp-content/themes/themefuse-parent/images/avatar_guest.jpg';
    ...
    function tfuse_get_instagram_photos($username, $items = 9) {
        if (false === ($instagram = get_transient('instagram-photos-'.sanitize_title_with_dashes($username)))) {
            ...
            if (is_wp_error($connect))
                return new WP_Error('site_down', __('Unable to communicate with Instagram.', 'tfuse'));
    
            if ( 200 != wp_remote_retrieve_response_code( $connect ) )
                return new WP_Error('invalid_response', __('Instagram did not return a 200.', 'tfuse'));
            ...
            if (!$instagram_array)
                return new WP_Error('bad_json', __('Instagram has returned invalid data.', 'tfuse'));
    
            $images = $instagram_array['entry_data']['UserProfile'][0]['userMedia'];
        ...
        if(!empty($body_font))
        {
            switch ($body_font) {
                ...
                case 'montstmerrat':
                    ...
                    break;
                default:
                    $font = 'http://fonts.googleapis.com/css?family=Lato:100,300,400,700,900,100italic,300italic,400italic,700italic,900italic';
        ...
        if(!empty($titles_font))
        {
            switch ($titles_font) {
                ...
                case 'montstmerrat':
                    ...
                    break;
                default:
                    $t_font = 'http://fonts.googleapis.com/css?family=Lato:100,300,400,700,900,100italic,300italic,400italic,700italic,900italic';
